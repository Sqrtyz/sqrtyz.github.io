...
+ 形式语言理论
    ...
    + 抽象化问题、语言
        ...
        可以把一个求答案的问题变成一个 0/1 decision 问题。

        定义语言 $L = \\{x \in \Sigma^{*}: Q(x) = 1 \\}$。

    + 在形式语言理论下，我们可以重新稍微形式化地去定义 P 和 NP 问题

        $$P = \\{L \subseteq \\{0,1\\}^{*}: \text{There exists an algorithm A that decides L in poly time}\\}$$

        此处 decide 意为算法 $A$ 必须要找到所有满足 $L$ 语言的 $x$，不重不漏。

        同时 

        $$NP = \\{L \subseteq \\{0,1\\}^{*}: \text{There exists a certificate } y \text{ with } |y|=O(|x|^c) \text{ such that } A(x,y)=1 \\}$$

        注意这个不知道从哪里蹦出来的解本身也得是多项式的。

        例如，我们可以用某种 01 语言来描述数独。定义 $Q(x)=1$ 当且仅当 $x$ 语言所描述的数独是正确的解。显然，不存在多项式算法 $A$ 来 decide 集合 $L$（因为这样就相当于在多项式时间内找到了所有的解），所以数独的求解不是一个 P 问题。

        但是当我们给定一个特定的解 $y$ 时，我们可以在多项式内确定 $A(x,y) = 1 \text{ or } 0$。所以数独的求解是一个 NP 问题。
